...
class StacContainer(object):
	...
	def putsinglefilefromstring(self, text, remotedestdir, remotefilename):
		'''
		This is convenient because the API does not offer a simple way to
		inject a single file into a running container (only a tar archive).

		This version takes a string instead of a local filename.
		'''
		# Make an in-memory tarfile that contains the given file
		import tarfile, StringIO
		tar_flo = StringIO.StringIO()
		tar = tarfile.open(mode='w', fileobj=tar_flo)
		flo = StringIO.StringIO(text)
		info = tarfile.TarInfo(name=remotefilename)
		info.size = len(flo.buf)
		info.mtime = time.time()
		tar.addfile(tarinfo=info, fileobj=flo)
		...
		tardata = tar_flo.getvalue()
		# Copy its contents to the container
		print("Copying %d-byte string into container %s in directory %s as %s" %
			(len(text), self.container.short_id, remotedestdir, remotefilename))
		success = self.container.put_archive(remotedestdir, tardata)
		assert success

	def isremotefile(self, remotepathname):
		'''
		Returns True if remotepathname exists and it's a regular file.
		This should be improved, but it's a hack that works for now.
		Beats not knowing and causing hard-to-find bugs!
		'''
		cmd = "test -f {} && echo EXISTS".format(remotepathname)
		output = self.execbashcmd(cmd)
		return 'EXISTS' in output

	def replaceallinfile(self, remotepathname, findwhat, replacewith):
		'''
		Replace all occurrences of findwhat with replacewith in the remote file.
		This should be improved to avoid any char-escaping problems, and to
		support regexps. Maybe we can use Python to subst instead of sed.
		'''
		assert self.isremotefile(remotepathname), "{} does not exist or is not a regular file".format(remotepathname)
		#pycmd = """python -c 'import re,sys\nf=open("{}", "r")\ntxt=f.read()\ntxt=re.subn("{}", "{}", txt)[0]\nf=open("{}", "w")\nf.write(txt)\nf.close()\n'""".format(
		#	remotepathname, findwhat, replacewith, remotepathname
		#)
		#print(pycmd)
		#self.execbashcmd(pycmd)
		#cmd = "sed -i ':a;N;$!ba;s/{}/{}/g' {}".format(findwhat, replacewith, remotepathname)
		#print(cmd)
		cmd = "sed -i 's/{}/{}/g' {}".format(findwhat, replacewith, remotepathname)
		self.execbashcmd(cmd)

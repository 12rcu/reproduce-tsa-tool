...
# This is exactly the same file provided in the examples directory
# except for the addition of the uploadRouteMapFromString method
# which we use to avoid temporary file creation during profiling.
...
class AirPlanClient:
    ...
    # Added by Nico
    def delete_first_map(self, session):
        import re
        pattern = re.compile(r'<input type="checkbox" name="routemap" value="([^"]+)">')
        url = self.get_root_url() + "delete_route_maps"
        ...
        value = m.group(1)
        url = self.get_root_url() + "delete_route_maps#"
        response2 = session.post(url, files={"routemap":value})
        assert response2.ok
        return response2
    ...
    def uploadRouteMapFromString(self, session, route_map_string, route_map_ext, route_map_name):
        """
        Uploads the route map contained in route_map_string to create a route map with the name route_map_name
        :param session:
        :param route_map_location:
        :param route_map_name:
        :return:
        ...
        assert route_map_ext in ('txt', 'xml', 'json')

        mimetype = 'text/plain' # default is text

        try:
            if route_map_ext=="xml":
                mimetype = 'text/xml'
            elif route_map_ext=="json":
                mimetype = 'application/json'
        except:
            pass # assume text

        response = session.post(url, files={'file' : (route_map_name, route_map_string, mimetype), 'route_map_name':route_map_name}, allow_redirects=True)
        self.propertiesURL = response.url
...
if __name__ == "__main__":
    ...
    with get_session() as session_:
        airplan.login(session_)
        if args.command=='upload':
            for route_map in args.route_map_file:
                print("uploading route map " + route_map)
                response_ = airplan.uploadRouteMap(session_, route_map, route_map.split('\\')[-1])
                print(response_.text)

        if args.command=='shortestpath':
            response_ = airplan.shortestPath(session_, args.src, args.dest, args.weight)
            print(response_.text)
        ...
        if args.command=='matrix':
            for route_map in args.route_map_file:
                print("uploading route map " + route_map)
                response_ = airplan.uploadRouteMap(session_, route_map, route_map.split('\\')[-1])
                response_ = airplan.properties(session_, "Cost") # user has to click through properties before viewing graph matrix
                response_ = airplan.passengerCapacity(session_, airplan.propertiesURL)
                print(response_.text)

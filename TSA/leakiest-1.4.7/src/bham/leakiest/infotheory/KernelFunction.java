...
public class KernelFunction {
	 ...
	 * @return estimated input distribution
	 */
	 ...
	 * 
	 * @param distInput input probability distribution
	 * @param Datalist list of observed data
	 * @return estimated mutual information
	 */
	public double calcContinuousApproxMI(double[] distInput, ArrayList<double[]> Datalist) {
		...
		for(int i = 0; i < resolutionForMI; i++) {
			...
			for(int x = 0; x < Datalist.size(); x++) {
				...
				if(probOoutputGivenInput[x] != ERROR)
					probOutput += distInput[x] * probOoutputGivenInput[x];
				else
					return ERROR;
			}

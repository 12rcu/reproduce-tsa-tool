...
public class PrintLeakageValue {
	...
	public PrintLeakageValue(boolean PRINT_CHANNELMATRIXin, boolean PRINT_JOINTMATRIXin, int verboseIn, double acceptableErrorIn, int noOfIterationsIn, boolean OBS_DISCRETEin, boolean readFromChanFileIn, boolean correctLeakIn, boolean correctLeakNewIn) {
	...
	private void printDiscreteCorrectedMI(ProbDist pd, Channel channel, int sampleSize) {
		...
		if( (channel.noOfInputs() <= 1) || channel.noOfOutputs() <= 1 ) {
			...
			if(!checkEachFeature || verbose > 1) {
				...
				System.out.println("  With 95% confidence, if estimated leakage < " + Stats.round(zeroUpperBound,4) + ", we may consider no information is leaked.");
	...
	private int printContinuousMutualInformation(ContinuousData cdata) {
		...
		final int noOfTests = 1000;
		int testSize = cdata.testSize;
		...
		//Calculate the estimated input distribution
		...
		if(!checkEachFeature || verbose > 1) {
			...
			System.out.printf(" (out of possible %6.4g bits)\n",
					  		  InfoTheory.log2(cdata.DataList.size()));
		}

		//
		// Run tests for 0 MI
		//
		double[] miResultsZ = new double[noOfTests];
		double miTotalZ = 0;
		...
		double averageZero = miTotalZ / noOfTests;
		...
		if(!checkEachFeature || verbose > 1) {
			if(verbose > 2) {
				System.out.print("For " + noOfTests + " using " + testSize + " SHUFFLED samples, ");
				System.out.println("mutual information is: ");
				System.out.print("  Average " + Stats.round(averageZero,4));
